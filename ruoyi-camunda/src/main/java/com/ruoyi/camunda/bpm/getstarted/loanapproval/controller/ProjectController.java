package com.ruoyi.camunda.bpm.getstarted.loanapproval.controller;

import com.ruoyi.camunda.bpm.getstarted.loanapproval.camunda.process.constant.ProjectProcessConstant;
...
public class ProjectController {
    ...
    /**
     * @return
     * @Title: getAllProjectParticipateRequest
     * <p>Description: </p>
     * @author: wqm
     * @Param: schoolName
     * @Param: reviewLevel
     * @version 创建时间：2021/3/24
     */

    @ApiImplicitParams({
            @ApiImplicitParam(name = "schoolName" , value = "学校名称" , required = true, paramType = "body"),
            @ApiImplicitParam(name = "reviewLevel" , value = "审批等级" , required = true, paramType = "body")
    })
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "needExtraInfo" , value = "是否需要附件" , required = true),
            @ApiImplicitParam(value = "是否通过" , required = true)}
    )
     ...
     */
    @ApiImplicitParams({
            @ApiImplicitParam(name = "userId" , value = "" , required = true, paramType = "Long")
    })
    ...
    public boolean uploadExtraInfo(@PathVariable Long projectId, @PathVariable Long userId, String extraInfo, String taskId) {
        ...
        Task task = taskService.createTaskQuery().
                taskAssignee(String.valueOf(userId)).//验证本人
                taskName(ProjectProcessConstant.TASK_NAME_UPLOAD_EXTRA_INFO).
                taskId(taskId).
                singleResult();
        if (task == null) {
